...
13. Hard requirement for the filled spec: instruct the executor to append to the implementation chat transcript as they go (after each completed step) and not fabricate it if saving is not possible.
14. If a required link or acceptance cases cannot be provided factually from inputs, ask for it as a clarifying question and stop with `=== WAITING FOR ANSWERS (EXECUTION-SPEC) ===`.
