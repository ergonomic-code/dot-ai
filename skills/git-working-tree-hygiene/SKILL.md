...
7. For task-created non-`src/` files (docs, configs, scripts), stage them with `git add -- <path>`.
8. For accidental or generated non-`src/` files, delete them or add them to `.gitignore` only if they are truly generated artifacts.
9. If a rename/move appears as a `D` entry plus an untracked `??` entry, stage both paths or redo the operation with `git mv`.
   Example: `git add -- <old-path> <new-path>`.
10. Re-run `git status --porcelain` and ensure the output matches expectations.
11. Run `git diff --name-status --cached` and ensure all intended changes for the next commit are staged.
12. Run `git diff --name-status` and ensure there are no intended changes left unstaged.
13. Before committing, run `git diff --stat --cached` and ensure only intentional files are included.
