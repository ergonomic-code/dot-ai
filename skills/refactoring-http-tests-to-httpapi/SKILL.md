...
description: Refactor Spring/JUnit HTTP tests to typed `*HttpApi` clients (controller-typed signatures, `*ForResponse`, optional `*ForError`, and JSON schema verification inside the client) when migrating away from direct WebTestClient/RestTestClient usage.
---
...
6. If the migrated tests include negative cases, implement `*ForError` that asserts the expected status, verifies the error schema, and returns a typed error representation or a response spec.
