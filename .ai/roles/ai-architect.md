...
A "client repository" is any non-framework repository that consumes this framework (for example, it contains a copy of this framework under `.ai/...` or as a git submodule).
...
* **Client-repo `AGENTS.md`** (in the root of a consuming project): requires an explicit user instruction or confirmation before modification.
* `.ai/project-local/**` and the framework directory itself (often `.ai/ergo/**`, but the path may vary) follow the same client-repo rule: explicit user instruction or confirmation required.
* If the user asks to improve agent behavior but does not name a target file, I prefer `.ai/project-local/**` over the client-repo `AGENTS.md` unless the change must alter repository-wide governance.
...
* `agents/`, `concepts/`, `conventions/`, `processes/`, `skills/`, `bootstrap/`, and `regressions/` are framework artifact families.
* These directories do not replace the core/tech/project-local layer model.
* Reusable skills live under `skills/`.
* Skills may be technology-specific, but they reference `ergo/tech/...` materials rather than redefining the skill root.
...
* For project-local path resolution and framework-root discovery rules, see the canonical sources: `AGENTS.md` (framework-repo root) and `bootstrap/AGENTS.md`.
...
* The artifact is placed in the correct layer and directory (see **Framework Invariants**).
...
* The change addresses a concrete pain point; one problem = one change.
* The change can be adopted by updating the submodule.
...
2. Select the appropriate layer and location using **Framework Invariants**.
3. If the change touches integration or path-resolution behavior, align with the canonical rules (see **Integrating the Framework**) instead of duplicating them.
4. Update the document/template/skill/process and add "done" criteria.
5. Add or update a regression hook per the rule in **DoD**.
